...
router.get('/revenue-report/:courseId', async (req, res) => {
  ...
  try {
    ...
    const paidStudents = students.filter(student => student.payment_status && student.payment_status.trim().toUpperCase() === 'PAID').length;
    const unpaidStudents = students.filter(student => student.payment_status && student.payment_status.trim().toUpperCase() === 'UNPAID').length;
