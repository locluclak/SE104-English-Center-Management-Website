...
const DashboardCharts = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await MainApiRequest.get("/course/all"); // Sử dụng MainApiRequest để lấy danh sách khóa học
  ...
  useEffect(() => {
    const fetchRevenueData = async () => {
      try {
        const response = await MainApiRequest.get(`/payment/revenue-report/${selectedCourseId}`); // Sử dụng MainApiRequest để lấy doanh thu
        ...
        const paidStudents = data.students.filter(student => student.payment_status.trim().toUpperCase() === 'PAID').length;
        const unpaidStudents = data.students.filter(student => student.payment_status.trim().toUpperCase() === 'UNPAID').length;
  ...
  useEffect(() => {
    const fetchTotalRevenueData = async () => {
      try {
        const response = await MainApiRequest.get('/payment/revenue-report-bytime', {
          params: {
  ...
  return (
    <div className="container-fluid m-2">
        ...
        <div className="header-actions d-flex me-3 py-2 align-items-center justify-content-between">
          <div className="flex-grow-1 d-flex justify-content-center">
            ...
            <Select
              ...
              onChange={handleCourseChange} // Khi thay đổi, gọi hàm này để cập nhật `selectedCourseId`
              options={courses.map((course) => ({
                label: course.name, // Hiển thị tên khóa học
                value: course.id,   // Lưu ID khóa học
              }))}
            />
          </div>
        </div>
      ...
      <div className="dashboard-charts-content" style={{ display: 'flex', flexDirection: 'column', gap: '40px', justifyContent: 'center' }}>
        <div style={{ width: '100%' }}>
